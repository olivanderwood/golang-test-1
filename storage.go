...
func (s *PostgresStore) CreateAccount(acc *Account) error {
	resp, err := s.db.Exec(`insert into account (first_name, last_name, number, balance, created_at)
							values
							($1, $2, $3, $4, $5)`, acc.FirstName, acc.LastName, acc.Number, acc.Balance)
	if err != nil {
		return err
	}
	fmt.Println(resp)
